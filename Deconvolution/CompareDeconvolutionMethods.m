    ...
    case 1 %generalizability within recording, within site across depths
        ...
        for i = 1:size(test_idx,1)                    
            ...
            if badprobe{train_idx(i,1)}(train_idx(i,2))==1 || badprobe{test_idx(i,1)}(test_idx(i,2))==1
                ...
                stats{i,5} = Deconvolve_Test(dff_test{test_idx(i,1)}(:,test_idx(i,2)),st_test{test_idx(i,1)}(:,test_idx(i,2)),'feedforward',trained_opts{train_idx(i,1)}(train_idx(i,2)));
                stats{i,6} = Deconvolve_Test(dff_test{test_idx(i,1)}(:,test_idx(i,2)),st_test{test_idx(i,1)}(:,test_idx(i,2)),'none',trained_opts{train_idx(i,1)}(train_idx(i,2)));
