function [W, H, cost,loadings,power] = seqNMF_camdentest(X, varargin)
%
% USAGE: 
%
...
smoothkernel = 0.01*ones(1,(2*L)-1);  
% smoothkernel = ones(1,(2*L)-1);  %ORIGINAL for factor competition
smallnum = max(X(:))*1e-6; 
lasttime = 0;
...
%         dHHdH = params.lambdaOrthoH*(~eye(K))*H;
    else
        dHHdH = 0;
    end
