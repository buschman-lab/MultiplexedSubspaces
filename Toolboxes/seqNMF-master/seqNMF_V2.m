...
W = params.W_init;
H = params.H_init;
if isa(X,'gpuArray'); W = gpuArray(W); H = gpuArray(H); end
...
smoothkernelH = 0.01*ones(1,(1*L)-1);  %ORIGINAL for factor competition SET to 4 for your stuff
smoothkernelW = 0.01*ones(1,(1*L)-1);  %ORIGINAL for factor competition
% smoothkernelH = 10*[(0.1:(1/(L-1)):1) 1 fliplr((0.1:(1/(L-1)):1))];
% smoothkernel = zeros(1,(2*L)-1);
smallnum = max(X(:))*1e-6; 
lasttime = 0;

% Calculate initial cost
cost = zeros(params.maxiter+1, 1);
if isa(X,'gpuArray'); cost = gpuArray(cost); end
...
for iter = 1 : params.maxiter
    ...
    if params.lambda>0
        dRdH = params.lambda.*(~eye(K))*conv2(WTX, smoothkernelW, 'same');  
    else 
        dRdH = 0; 
    end
    if params.lambdaOrthoH>0
        dHHdH = params.lambdaOrthoH*(~eye(K))*conv2(H, smoothkernelH, 'same');
    else
        dHHdH = 0;
    end
    ...
    if params.showPlot 
        if isa(Xhat,'gpuArray')
            SimpleWHPlot(gather(W), gather(H), gather(Xhat),0);
        else
            SimpleWHPlot(W, H, Xhat,0); 
        end
        title(sprintf('training lvl #%i : iteration #%i',params.Trainlvl,params.cur_iter));
        drawnow
    end
...
if params.SortFactors
    ...
    function [X,N,T,K,L,params] = parse_seqNMF_params(X, inputs);
        ...
        addOptional(p,'cur_iter',1); %It interating on multiple recs
        addOptional(p,'Trainlvl',1); %If training on multiple recordings 
        addOptional(p,'K',10);
        addOptional(p,'L',100);
        ...
        addOptional(p,'W_init', []); % depends on K--initialize post parse
        addOptional(p,'H_init', []); % depends on K--initialize post parse
