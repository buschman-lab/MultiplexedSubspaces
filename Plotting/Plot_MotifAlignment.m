...
cur_rec = 6;
cur_motif = [5,6];
area  = {'VIS','PRE','SSp-bfd'}; %important | this should always go SHARED --> Higher in Motif A --> Higher in Motif B; 
...
theta = arrayfun(@(x) theta(x,:),1:size(theta,1),'UniformOutput',0);
theta_all = {theta(:), x(:),y(:),z(:),q(:),r(:),w(:)};
xpos=[1,3,4,5,6,7,8];
%%
% plotCombinedSubspaceAngles(theta_all,xpos)
%%
plotSubspaceAngles(theta)
%%
saveCurFigs(get(groot, 'Children'),{'-dsvg','-dpng'},sprintf('CombinedStatistics'),savedir,0); close all
...
for i = 1:numel(xx)
    ...
    text(xpos(i),3,sprintf('%0.2f +/- %0.2f \n[%0.2f %0.2f] \n p=%0.4f',mu,err,ci(1),ci(2),p),'FontSize',5);
    CompareViolins(thetaboot',fp,'plotspread',0,'divfactor',1,'xpos',xpos(i));    
end
xlim([0.5 xpos(end)+0.5])
plot([0 xpos(end)],[0 0],'linestyle','-','color',[0.3 0.3 0.3],'linewidth',1.5)
yvals = get(gca,'ylim');
ylim([floor(yvals(1)),max(ceil(yvals(2)),10)]);
ylabel('\Delta theta')
fp.FormatAxes(gca)
fp.FigureSizing(gcf,[3 3 8 4],[10 10 14 10])
end
...
%make sure recording actually fits the criteria of higher activity in one
% and lower in the other. 
s = cell(1,6);
for cur_rec = 1:6
    try
        s{cur_rec} = CompareActivity(data,cur_rec,cur_motif,area,0); 
    catch
    end
end
idx = cellfun(@(x) isempty(x), s);
s(idx)=[];
crit_rec = cellfun(@(x) sum(x)==1,s); %recording meets the criteria of one region having more activity and the other not. 
...
figure; hold on;
histogram(theta,'facecolor',[0.5 0.5 0.5],'edgecolor','none','BinWidth',5)
yvals = get(gca,'ylim');
...
function x = GetSubspaceAngleAcrossRecordings(data,cur_motif,area)
...
function s = CompareActivity(data,cur_rec,cur_motif,area,verbose)
