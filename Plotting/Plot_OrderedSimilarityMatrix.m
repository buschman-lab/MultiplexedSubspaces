...
opts.label_col = getColorPalet(numel(unique(clust_id)));
...
for i = 1:numel(opts.order)
    ...
    %Add X label
    text(opts.offset*-6,mean(tickval),['\color[rgb]{' num2str(opts.label_col(i,:)),sprintf('}%d',i)],'HorizontalAlignment','Center','FontSize',16)   
    %Add Y label
    text(mean(tickval),numel(clust_id)+6*opts.offset,['\color[rgb]{' num2str(opts.label_col(i,:)),sprintf('}%d',i)],'rotation',0,'HorizontalAlignment','Center','FontSize',16)
    %Add colored lines 
