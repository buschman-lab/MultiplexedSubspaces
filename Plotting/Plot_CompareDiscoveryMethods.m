...
%grouping variable
group = [repmat({'sPCA'},1,numel(spatial_pca)),repmat({'sNMF'},1,numel(spatial_nmf))];

%cnmf
cnmf_ev = cat(1,cnmf(:).ExpVar_all)*100; 
cnmf_dim = cell2mat(cat(1,cnmf(:).numFactors)); 

group_sub = [repmat({'sPCA'},1,numel(spatial_pca)),repmat({'sNMF'},1,numel(spatial_nmf)),repmat({'Motifs'},1,numel(cnmf_dim))];
...
g(1,1)=gramm('x',num_components,'y',cat(2,spatial_pca,spatial_nmf,{zeros(1,numel(num_components))}),'color',[group,{'Motifs'}]);
...
g(2,1)=gramm('x',cat(1,spatial_pca_dim',spatial_nmf_dim',cnmf_dim),'color',group_sub);
