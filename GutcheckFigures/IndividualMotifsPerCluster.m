function IndividualMotifsPerCluster(data_dir,basis_name,save_dir)
%camden macdowell - timeless

% data_dir = 'Z:\Rodent Data\Wide Field Microscopy\VPA Experiments_Spring2018\VPA_Mesomapping\FitMotifs_deconvolution_aic';
% basis_name = 'ALL_10-stv';
% save_dir = [data_dir filesep 'GutCheckData']

%load the cluster
load([data_dir filesep basis_name filesep 'basismotifs.mat'],'W_basis');
...
for i = 1:K
   ...
   MotifToGif(temp,[save_dir filesep sprintf('BM%d.gif',i)],'limit',99); 
   
   %Get up to 20 random motifs that are part of this cluster
   temp_idx = find(cluster_idx==i);
   if numel(temp_idx)>20
       temp_idx = temp_idx(randperm(numel(temp_idx),20));
   end
   indi_motif = W(:,temp_idx,:);
   for j = 1:size(indi_motif,2)
       temp = squeeze(indi_motif(:,j,:));
       temp = reshape(temp,[68 68 size(temp,2)]);
       MotifToGif(temp,[save_dir filesep sprintf('BM%d_Example%d.gif',i,j)],'limit',99); 
   end   
end %basis loop
