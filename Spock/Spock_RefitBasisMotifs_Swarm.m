function Spock_RefitBasisMotifs_Swarm(file_list,basis_dir,job_id,s_conn,parameter_class)
%Camden MacDowell
...
for cur_file = 1:numel(file_list)     
    ...
    for cur_chunk = 1:size(temp.data_test,3)
        script_name = WriteBashScript(sprintf('%d',1),'RefitBasisMotifs',{ConvertToBucketPath(file_list{cur_file}),ConvertToBucketPath(basis_dir),cur_chunk,parameter_class},...
        {"'%s'","'%s'",'%d',"'%s'"},...
