...
if nargin<2
    save_dir = '/TrainRepitoires/SpaceTimeNMF_Sweep/';
end
save_file_name = sprintf('block_%d',block);

%% Body
% [X_recon,err,Wt,A,Ws, fit_to_data, opts,dimensions,pev,rank_a] = DecomposeMotifs('block',block);
[fit_to_data,fit_full,opts]= DecomposeMotifs('block',block);
...
save(filename,'fit_full','fit_to_data','-v7.3');
