...
split_idx = {12,10.00,1.78}; %mouse 431
...
%% Tabulate clusters
features_binned = ceil(movmedian(features_binned,1*13));
...
bad_states = unique_states(clusters(:,num_features+1)<=ceil(2/100*numel(indx_clusters)));
...
for i = 1:size(cluster_group_clean,1)
   ...
   for j = 1:size(temp,2)
      ...
      avg_h_classification{i}(j,:) = nanmean(test);
   end
end
...
if ~isempty(bad_states)
    ...
    avg_h_classification(num_states:end)=[];
    cluster_group_clean(num_states:end,:)=[];
end
...
figure; hold on;
temp = cellfun(@ numel,cluster_group_clean,'UniformOutput',0);
temp = cell2mat(temp)/13;
temp(temp==0)=NaN;
boxplot(temp')
title({'Instance Duration (sec)'},'FontName','Arial','FontSize',16,'FontWeight','normal');
...
fp = fig_params;
sig_color = [0.25 0.25 0.25;0.75 0.75 0.75];
...
for i = 1:numel(sig_comparison)
   ...
   for j = 1:numel(temp)
       plot(i,temp(j),'.','markersize',25,'color',[0.2 0.7 1]);       
...
not_significant = cellfun(@ isempty,sig_comparison,'UniformOutput',0);
not_significant = cell2mat(not_significant); 
scatter((1:1:size(temp_avg,2)),fstat,50,sig_color(not_significant+1,:),'filled')
