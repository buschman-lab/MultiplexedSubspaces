function stats = BehavioralStateClassification_avg(weight,num_states)
...
comparisons = nchoosek(1:num_states,2);
...
for i = 1:size(comparisons,1)
   data1 = (cell2mat(weight(:,comparisons(i,1))));
   
   data2 = (cell2mat(weight(:,comparisons(i,2))));
   
   %remove columns with nans
   data1(:,~any(~isnan(data1)))=[];
   data2(:,~any(~isnan(data2)))=[];
   
   len_min = min([size(data1,2),size(data2,2)]);
   
   %grab random samples from both; 
   data1 = data1(:,randperm(size(data1,2),len_min));
   data2 = data2(:,randperm(size(data2,2),len_min));
   ...
   %set to zero and 1
   [~, Observed, ~] = SVMClassifier_Binary([cat(1,data1',data2'),temp_resp],[],'holdout',cur_round/10,...
      'nshuf',0,'featureselect','none','optimize',1,'pca',0,'solver',1,'kernel','rbf','numkfold',5);
...
plot(x,y,'color',[0.5 0.5 0.5],'linestyle','none','marker','.','markersize',25);
