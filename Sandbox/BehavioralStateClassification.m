function stats = BehavioralStateClassification(avg_h_classification)
...
%Loop through all binary classifiers
comparisons = nchoosek(unique(labels(:)),2);
...
for i = 1:size(comparisons,1)
   ...
   idx = labels==comparisons(i,1)|labels==comparisons(i,2); 
  
   %get the final 0.2 indices
   cvp.training = logical([ones(1,n_train), zeros(1,n_test) ones(1,n_train), zeros(1,n_test)]);
   cvp.test = logical([zeros(1,n_train), ones(1,n_test) zeros(1,n_train), ones(1,n_test)]);
      
   %binarize   
   temp_resp = NaN(numel(labels(idx)),1);
   temp_resp(labels(idx)==comparisons(i,1),1)= 1;   
   temp_resp(isnan(temp_resp(:,1)))=2;

   %set to zero and 1
   [~, Observed, ~] = SVMClassifier_Binary([classification_data(idx,:),temp_resp],cvp,'holdout',cur_round/10,...
      'nshuf',0,'featureselect','none','optimize',1,'pca',0,'solver',1,'kernel','rbf','numkfold',3);
   ...
   cm(comparisons(i,1),comparisons(i,2))=temp_cm(2,1)/sum(temp_cm(2,:));
   cm(comparisons(i,2),comparisons(i,1))=temp_cm(1,2)/sum(temp_cm(1,:));
end
cm = round(cm,2);
cm(cm==0)=NaN;
...
set(gca,'ylim',[0.4 0.7],'xlim',[-0.3 .3],'xtick','')
setFigureDefaults
